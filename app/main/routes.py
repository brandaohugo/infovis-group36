...
from app import models, plots, data, map_data
...
def map():
    return render_template("map.html", map_data=map_data.us_states_map, airport_locations=map_data.us_airport_locations)
...
def bokeh():
    ...
    return render_template("bokeh.html",
                           all_property_types=data.all_property_types,
                           all_property_types_text=data.all_property_types_text,
...
def airline():
	...
	table_data = pd.pivot_table(table_data, index = 'MONTH', columns = 'AIRLINE', values = 'ARR_DELAY').mean().round(2).to_frame().rename(columns={0:"AVG_ARR_DELAY"}).reset_index().sort_values(by=['AVG_ARR_DELAY'])
